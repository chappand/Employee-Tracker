...
const viewEmployees = () => {
    ...
    connection.query(
        'SELECT id, first_name, last_name, department_name, title, salary, manager_id FROM employee_role LEFT JOIN department ON employee_role.department_id = department.id LEFT JOIN employee ON employee_role.id = employee.role_id', (err, res) => {
            if (err) throw err;
            console.table(res);
            startInquiry();
        }
